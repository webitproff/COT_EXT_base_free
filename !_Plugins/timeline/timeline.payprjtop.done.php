...
global $cfg, $usr;
if ($cfg['plugin']['timeline']['payprjtop_done'] != 'none')
{
  ...
  $tltext = cot_rc($L['timeline_payprjtop_done'], array(
						'user_name' => $advusr['user_name'],
            'item_title' => $adv['item_title'],
            'item_url' => cot_url('projects', $urlparams, '', true)
					));
  cot_add_timeline($pay['pay_userid'], $pay['pay_code'], 'payprjtop', 'done', $tltext);
}