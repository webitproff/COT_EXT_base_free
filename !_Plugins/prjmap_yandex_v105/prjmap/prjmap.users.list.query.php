...
if($showmapradius && $_GET['mapradius'] > 0 && $_GET['city'] > 0) {
  ...
  if( $xml->status == 'OK' ) {
    ...
    $where['location'] = '('.(!empty($where['location']) ? $where['location'].' OR ' : '').'(user_prjmaplat!=0 AND user_prjmaplng!=0 AND prjmapdist('.$lat.', '.$lng.', user_prjmaplat, user_prjmaplng)<'.$_GET['mapradius'].'))';
    ...
    $join_columns .= ', prjmapdist('.$lat.', '.$lng.', user_prjmaplat, user_prjmaplng) as mapdistance';
  }
}
