...
/**
 * [BEGIN_COT_EXT]
 * Hooks=market.edit.update.error
 * [END_COT_EXT]
 */
...
if(!empty($ritem['market_cat'])) {
  ...
  if($checkextrafields['chk_id'] > 0) {
    ...
    if(in_array('files', $checkextrafields['chk_set']) && cot_module_active('files') && $item['market_id'] > 0) {
       cot_check($db->query("SELECT COUNT(*) FROM $db_files WHERE file_source='market' AND file_item=".$item['market_id'])->fetchColumn() == 0, $L['checkextrafields_market_files']);
    }
    if(in_array('mavatars', $checkextrafields['chk_set']) && cot_plugin_active('mavatars') && $item['market_id'] > 0) {
       $mavatar = new mavatar('market', $ritem['market_cat'], $item['market_id'], 'edit');
