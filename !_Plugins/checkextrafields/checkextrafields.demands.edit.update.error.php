...
/**
 * [BEGIN_COT_EXT]
 * Hooks=demands.edit.update.error
 * [END_COT_EXT]
 */
...
if(!empty($ritem['item_cat'])) {
  ...
  if($checkextrafields['chk_id'] > 0) {
    ...
    if(in_array('files', $checkextrafields['chk_set']) && cot_module_active('files') && $item['item_id'] > 0) {
       cot_check($db->query("SELECT COUNT(*) FROM $db_files WHERE file_source='demands' AND file_item=".$item['item_id'])->fetchColumn() == 0, $L['checkextrafields_demands_files']);
    }
    if(in_array('mavatars', $checkextrafields['chk_set']) && cot_plugin_active('mavatars') && $item['item_id'] > 0) {
       $mavatar = new mavatar('demands', $ritem['item_cat'], $item['item_id'], 'edit');
