...
cot_blockguests() || cot_block((int)$cfg['plugin']['amarket']['am_custumer_id'] == (int)$usr['maingrp']);
...
if($n == "forconfirm"){
	$id_status = 1;
	$invisible = ' AND amo_invisible_customer <> 1';
...
$sqlarray = $db->query("SELECT * FROM {$db_amarket_orders} WHERE amo_status = {$id_status} AND amo_customer =".(int)$usr['id']." {$invisible} {$sql_sort} LIMIT {$d}, " . $maxrowsperpage)->fetchAll();
$totalitems = $db->query("SELECT COUNT(*) as total FROM {$db_amarket_orders} WHERE amo_status = {$id_status} {$invisible} AND amo_customer =".(int)$usr['id'])->fetchColumn();
...
foreach ($sqlarray as $row) {
	...
	if($n == "waitpayment"){
		if($cfg['plugin']['amarket']['am_enablereason']){
			...
			$cancel_url = cot_url('index', 'r=amarket&m=myorders&s=cancel&amo_id='.$row['amo_id']."&".cot_xg(),'',true);
			$cancel_bnt = cot_rc('amarket_btn_change_status_cancel', array('url' => $cancel_url, 'id' => $row['amo_id'], 'text' => $L['Cancel_order']));
		}
		$pay_url =  cot_url('amarket', 'm=pay&amo_id='.$row['amo_id']."&".cot_xg(),'',true);
		$pay_btn = cot_rc('amarket_btn_pay_confirm', array('url' => $pay_url, 'id' => $row['amo_id'], 'text' => $L['Pay_order']));
	}

	if($n == "cancelled" || $n == "paid"){
		$invisible_btn_url =  cot_url('index', 'r=amarket&m=myorders&s=invisible&amo_id='.$row['amo_id']."&".cot_xg(),'',true);
...
if(($n == "cancelled" || $n == "paid") && $totalitems > 0){
	$invisible_btn_all_url = cot_url('amarket', 'm=myorders&n='.$n.'&a=invisible&'.cot_xg(),'',true);
...
$orders_count = $db->query("SELECT amo_status, COUNT(amo_status) as total  FROM {$db_amarket_orders} WHERE amo_customer =".(int)$usr['id']." {$invisible} GROUP BY amo_status")->fetchAll(PDO::FETCH_GROUP);
$t->assign(array(
	...
	'AMARKET_MYORDERS_FORCONFIRM' 	=> cot_url('amarket', 'm=myorders&n=forconfirm'),
	'AMARKET_MYORDERS_WAITPAYMENT'	=> cot_url('amarket', 'm=myorders&n=waitpayment'),
	'AMARKET_MYORDERS_PAID' 		=> cot_url('amarket', 'm=myorders&n=paid'),
	'AMARKET_MYORDERS_CANCELLED' 	=> cot_url('amarket', 'm=myorders&n=cancelled'),
	'AMARKET_MYORDERS_FORCONFIRM_COUNT' 	=> ($orders_count['1']) ? $orders_count['1'][0]['total'] : 0 ,
	'AMARKET_MYORDERS_WAITPAYMENT_COUNT'	=> ($orders_count['2']) ? $orders_count['2'][0]['total'] : 0 ,
	'AMARKET_MYORDERS_PAID_COUNT' 			=> ($orders_count['3']) ? $orders_count['3'][0]['total'] : 0 ,
	'AMARKET_MYORDERS_CANCELLED_COUNT' 		=> ($orders_count['4']) ? $orders_count['4'][0]['total'] : 0 ,
	));
//повідомлення
