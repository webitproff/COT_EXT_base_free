...
if($a == 'add')
{
 ...
 if (!cot_error_found())
    ...
    $wall['item_id'] = $db->lastInsertId();

    if (cot_plugin_active('mavatars'))
    {
    	require_once cot_incfile('mavatars', 'plug');

    	$mavatar = new mavatar('userwall', $wall['item_userid'], $wall['item_id'], 'edit');
    	$mavatar->update();
    	$mavatar->upload();
    }
...
elseif($a == 'del' && $id > 0)
{
  ...
  {
		$db->delete($db_userwall, "item_id=".$wall['item_id']);

    if (cot_plugin_active('mavatars'))
    {
    	require_once cot_incfile('mavatars', 'plug');
    	$mavatar = new mavatar('userwall', $wall['item_userid'], $wall['item_id']);
...
elseif($a == 'edit' && $id > 0 && COT_AJAX)
{
  ...
  if($wall['item_id'] > 0 || $usr['maingrp'] == 5 || $usr['maingrp'] == 6)
  {
	  echo '<form class="uk-form" name="newwallpost" action="'.cot_url('plug', 'r=userwall&a=update&id='.$wall['item_id']).'" method="post" enctype="multipart/form-data">
             <fieldset>
