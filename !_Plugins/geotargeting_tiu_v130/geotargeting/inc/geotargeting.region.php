...
if ($a == 'del')
{
	...
	$cache && $cache->clear();
	cot_redirect(cot_url('admin', 'm=other&p=geotargeting&n=region&country=' . $country . '&d=' . $d_url, '', true));
	exit;
}
...
$t = new XTemplate(cot_tplfile('geotargeting.region', 'plug'));
...
$pagenav = cot_pagenav('admin', "m=other&p=geotargeting&n=region&country=" . $country, $d, $totalitems, $cfg['maxrowsperpage']);
...
while ($item = $sql->fetch())
{
	...
	$t->assign(array(
		...
		"REGION_ROW_URL" => cot_url('admin', 'm=other&p=geotargeting&n=city&id=' . $item['region_id']),
		"REGION_ROW_DEL_URL" => cot_url('admin', 'm=other&p=geotargeting&n=region&country=' . $country . '&a=del&rid=' . $item['region_id']),
		"REGION_ROW_NUM" => $jj,
...
$t->assign(array(
	"ADD_FORM_ACTION_URL" => cot_url('admin', 'm=other&p=geotargeting&n=region&country=' . $country . '&a=add'),
	"ADD_FORM_NAME" => cot_inputbox('text', 'rname', ''),
));
$t->parse("MAIN.ADDFORM");

$t->assign(array(
	"EDIT_FORM_ACTION_URL" => cot_url('admin', 'm=other&p=geotargeting&n=region&country=' . $country . '&a=edit&d=' . $d_url),
...
$adminpath[] = array(cot_url('admin', 'm=other&p=geotargeting&n=region&country=' . $country), $cot_countries[$country]);
