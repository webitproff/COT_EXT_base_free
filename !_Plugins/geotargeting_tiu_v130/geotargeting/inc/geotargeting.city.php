...
if ($a == 'del')
{
	...
	$cache && $cache->clear();
	cot_redirect(cot_url('admin', 'm=other&p=geotargeting&n=city&id=' . $id, '', true));
	exit;
}
...
$t = new XTemplate(cot_tplfile('geotargeting.city', 'plug'));
...
$pagenav = cot_pagenav('admin', "m=other&p=geotargeting&n=city&id=" . $id, $d, $totalitems, $cfg['maxrowsperpage']);
...
while ($item = $sql->fetch())
{
	...
	$t->assign(array(
		...
		"CITY_ROW_DEL_URL" => cot_url('admin', 'm=other&p=geotargeting&n=city&id=' . $id . '&a=del&cid=' . $item['city_id']),
	));
...
$t->assign(array(
	...
	"ADD_FORM_ACTION_URL" => cot_url('admin', 'm=other&p=geotargeting&n=city&id=' . $id . '&a=add', '', true),
	"ADD_FORM_TITLE" => $title,
));
$t->parse("MAIN.ADDFORM");

$t->assign(array(
	"EDIT_FORM_ACTION_URL" => cot_url('admin', 'm=other&p=geotargeting&n=city&id=' . $id . '&a=edit&d=' . $d_url, '', true),
...
$adminpath[] = array(cot_url('admin', 'm=other&p=geotargeting&n=region&country=' . $region['region_country']), 
	$cot_countries[$region['region_country']]);
$adminpath[] = array(cot_url('admin', 'm=other&p=geotargeting&n=city&id=' . $region['region_name']), $region['region_name']);
$t->parse("MAIN");
