...
<!-- BEGIN: ADMIN_COMMENTS_ROW -->
				<tr>
					...
					<td class="centerall action">
						<!-- IF {PHP.row.com_state} == 1 --><a title="{PHP.L.Validate}" href="{ADMIN_COMMENTS_ITEM_VALIDATE_URL}" class="confirmLink button">{PHP.L.Validate}</a><!-- ENDIF -->
						<!-- IF {PHP.row.com_state} == 0 --><a title="{PHP.L.Lock}" href="{ADMIN_COMMENTS_ITEM_UNVALIDATE_URL}" class="confirmLink button">{PHP.L.Lock}</a><!-- ENDIF -->
