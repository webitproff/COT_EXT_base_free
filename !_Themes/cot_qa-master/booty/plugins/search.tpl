			...
			<h2 class="stats">{PLUGIN_TITLE}</h2>
				<div>
					<form id="search" name="search" action="{PLUGIN_SEARCH_ACTION}" method="get">
						...
						<!-- IF {PHP.cfg.plugin.search.forumsearch} AND {PHP.cfg.plugin.search.pagesearch} -->
						<p class="textcenter">
							<a href="{PHP|cot_url('plug','e=search')}">{PHP.L.plu_tabs_all}</a> |
							<!-- IF {PHP.cfg.plugin.search.forumsearch} -->
							<a href="{PHP|cot_url('plug','e=search&amp;tab=frm')}">{PHP.L.Forums}</a> |
							<!-- ENDIF -->
							<!-- IF {PHP.cfg.plugin.search.pagesearch} -->
						...
						<div class="well">
							<div class="row">
								<div class="span12">
									...
									{PLUGIN_SEARCH_TEXT} 
									<button class="btn btn-primary">{PHP.L.plu_search_key}</button>
									</div>
								</div>
							</div>
